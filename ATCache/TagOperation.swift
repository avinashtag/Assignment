...
public typealias TagOperationCompletionBlock = (_ operation: TagOperation, _ result: DownloadCompletion) -> ()

public class TagOperation: Operation, URLSessionDelegate, URLSessionTaskDelegate, URLSessionDataDelegate, URLSessionDownloadDelegate {
    ...
    // a completion block
    private let finishedBlock: TagOperationCompletionBlock
    ...
    public init(urlRequest request: URLRequest, identifier id: String, callbackQueue: DispatchQueue, downloadFinishedBlock:  @escaping TagOperationCompletionBlock) {
        urlRequest = request
        identifier = id
    ...
    public func urlSession(_ session: URLSession, downloadTask: URLSessionDownloadTask, didFinishDownloadingTo location: URL) {
        ...
        if (200...299).contains(response.statusCode) {
            if let data = try? Data(contentsOf: location), let image = UIImage(data: data) {
                ...
                finishedCallbackQueue.async {
                    self.finishedBlock(self, .error(error: NSError(domain: Tag.domain, code: 0, userInfo: [NSLocalizedDescriptionKey: "unable to decode image"])))
                }
            }
        } else {
            finishedCallbackQueue.async {
                self.finishedBlock(self, .error(error: NSError(domain: Tag.domain, code: response.statusCode, userInfo: [NSLocalizedDescriptionKey: "unexepected status code \(response.statusCode)"])))
            }
        }
    }
